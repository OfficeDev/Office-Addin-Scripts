...
export async function configureSSO(manifestPath: string) {
  ...
  const port: number =
    parseDevServerPort(process.env.npm_package_config_dev_server_port) || 3000;
  ...
  if (Object.keys(userJson).length >= 1) {
    ...
    if (applicationJson) {
      ...
      if (await configure.isUserTenantAdmin(userJson)) {
        ...
        const setSharePointReplyUrls: boolean =
          await configure.setSharePointTenantReplyUrls(
        ...
        const setOutlookReplyUrl: boolean =
          await configure.setOutlookTenantReplyUrl();
      ...
      const secret: string = await configure.setApplicationSecret(
        applicationJson
      );
    ...
    console.log(
      chalk.green(
        `Application with id ${applicationJson["appId"]} successfully registered in Azure.  Go to https://ms.portal.azure.com/#home and search for 'App Registrations' to see your application`
      )
    );
