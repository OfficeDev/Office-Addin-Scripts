...
export async function createNewApplication(
  ...
  try {
    let azRestCommand = await fs.readFileSync(
      defaults.azRestAppCreateCommandPath,
      "utf8"
    );
    const reName = new RegExp("{SSO-AppName}", "g");
    const rePort = new RegExp("{PORT}", "g");
    azRestCommand = azRestCommand
      .replace(reName, ssoAppName)
      .replace(rePort, port);
...
export async function grantAdminContent(
  ...
  try {
    ...
    switch (process.platform) {
      case "win32": {
        ...
        const appsWindows: any = await promiseExecuteCommand(
          appsInstalledWindowsCommand
        );
        cliInstalled =
          appsWindows.filter((app) => app.DisplayName === "Microsoft Azure CLI")
            .length > 0;
        // Send usage data
...
export async function isUserTenantAdmin(userInfo: Object): Promise<boolean> {
  try {
    let azRestCommand: string = fs.readFileSync(
      defaults.azRestGetTenantRolesPath,
      "utf8"
    );
    ...
    azRestCommand = fs.readFileSync(
      defaults.azRestGetTenantAdminMembershipCommandPath,
      "utf8"
    );
...
export async function setApplicationSecret(
  ...
  try {
    let azRestCommand: string = await fs.readFileSync(
      defaults.azRestAddSecretCommandPath,
      "utf8"
    );
    azRestCommand = azRestCommand.replace(
      "<App_Object_ID>",
      applicationJson["id"]
    );
...
export async function setIdentifierUri(
  applicationJson: Object,
  port: string
): Promise<void> {
  try {
...
export async function setSignInAudience(
  applicationJson: Object
): Promise<void> {
  try {
    let azRestCommand: string = fs.readFileSync(
      defaults.azRestSetSigninAudienceCommandPath,
      "utf8"
    );
    azRestCommand = azRestCommand.replace(
      "<App_Object_ID>",
...
export async function setSharePointTenantReplyUrls(
  tenantName: string
): Promise<boolean> {
  try {
    ...
    if (setReplyUrls) {
      azRestCommand = fs.readFileSync(
        defaults.azRestAddTenantReplyUrlsCommandPath,
        "utf8"
      );
      const reName = new RegExp("<TENANT-NAME>", "g");
      azRestCommand = azRestCommand
        .replace(reName, tenantName)
        .replace("<SP-OBJECTID>", servicePrinicipaObjectlId);
      await promiseExecuteCommand(azRestCommand);
    }
...
export async function setOutlookTenantReplyUrl(): Promise<boolean> {
  try {
    ...
    if (setReplyUrls) {
      azRestCommand = fs.readFileSync(
        defaults.azRestAddTenantOutlookReplyUrlsCommandPath,
        "utf8"
      );
      azRestCommand = azRestCommand.replace(
        "<SP-OBJECTID>",
        servicePrinicipaObjectlId
      );
      await promiseExecuteCommand(azRestCommand);
    }
