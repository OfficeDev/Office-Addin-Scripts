...
function determineManifestPath(platform: Platform, dev: boolean): string {
  ...
  if (!fs.existsSync(manifestPath)) {
    throw new ExpectedError(
      `The manifest path does not exist: ${manifestPath}.`
    );
  }
...
function parseDevServerPort(optionValue: any): number | undefined {
  const devServerPort = parseNumber(
    optionValue,
  ...
  if (devServerPort !== undefined) {
    ...
    if (devServerPort < 0 || devServerPort > 65535) {
      throw new ExpectedError(
        "--dev-server-port should be between 0 and 65535."
      );
    }
  }
...
export async function start(
  ...
  try {
    const appPlatformToDebug: Platform | undefined = parsePlatform(
      platform ||
        process.env.npm_package_config_app_platform_to_debug ||
        Platform.Win32
    );
    const appTypeToDebug: AppType | undefined = devSettings.parseAppType(
      appPlatformToDebug ||
        process.env.npm_package_config_app_type_to_debug ||
        AppType.Desktop
    );
    const appToDebug: string | undefined =
      command.app || process.env.npm_package_config_app_to_debug;
    const app: OfficeApp | undefined = appToDebug
      ? parseOfficeApp(appToDebug)
      : undefined;
    ...
    const devServer: string | undefined =
      command.devServer || (await getPackageJsonScript("dev-server"));
    const devServerPort = parseDevServerPort(
      command.devServerPort || process.env.npm_package_config_dev_server_port
    );
    const document: string | undefined =
      command.document || process.env.npm_package_config_document;
    ...
    const packager: string | undefined =
      command.packager || (await getPackageJsonScript("packager"));
    const packagerHost: string | undefined =
      command.PackagerHost || process.env.npm_package_config_packager_host;
    const packagerPort: string | undefined =
      command.PackagerPort || process.env.npm_package_config_packager_port;
    ...
    if (
      appPlatformToDebug === Platform.Android ||
      appPlatformToDebug === Platform.iOS
    ) {
      throw new ExpectedError(
...
export async function stop(
  ...
  try {
    const appPlatformToDebug: Platform | undefined = parsePlatform(
      platform ||
        process.env.npm_package_config_app_plaform_to_debug ||
        Platform.Win32
    );
    const dev: boolean = command.prod ? false : true;
