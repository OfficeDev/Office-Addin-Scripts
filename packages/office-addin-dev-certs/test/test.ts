    ...
    describe("install-tests", function() {
        ...
        if (process.platform === "win32") {
            it("with --machine option", async function() {
                ...
                assert.strictEqual(execSync.callCount, 1);
                assert.strictEqual(execSync.calledWith(`powershell scripts\\install.ps1 LocalMachine '${testCaCertificatePath}'`), true);
            });
            it("without --machine option", async function() {
                ...
                await install.installCaCertificate(testCaCertificatePath, machine);
                assert.strictEqual(execSync.callCount, 1);
                assert.strictEqual(execSync.calledWith(`powershell scripts\\install.ps1 CurrentUser '${testCaCertificatePath}'`), true);
            });
        }
    });
    describe("uninstall-tests", function() {
        ...
        if (process.platform === "win32") {
            it("with --machine option", async function() {
                ...
                assert.strictEqual(execSync.callCount, 1);
                assert.strictEqual(execSync.calledWith(`powershell scripts\\uninstall.ps1 LocalMachine '${defaults.caCertificatePath}' '${defaults.certificateName}'`), true);
            });
            it("without --machine option", async function() {
                const isCaCertificateInstalled = sandbox.fake.returns(true);
                const execSync = sandbox.fake();
                ...
                await uninstall.uninstallCaCertificate(machine);
                assert.strictEqual(execSync.callCount, 1);
                assert.strictEqual(execSync.calledWith(`powershell scripts\\uninstall.ps1 CurrentUser '${defaults.caCertificatePath}' '${defaults.certificateName}'`), true);
            });
        }
    });
