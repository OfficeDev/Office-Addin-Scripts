...
describe("test json output", function () {
  describe("generate test", function () {
    it("test it", async function () {
      ...
      assert.strictEqual(
        j.functions[1].result.dimensionality,
        "matrix",
        "result dimensionality matrix not created properly"
      );
      ...
      assert.strictEqual(
        j.functions[10].options.stream,
        true,
        "CustomFunctions.StreamingHandler - options stream not created properly"
      );
      assert.strictEqual(
        j.functions[10].result.type,
        "number",
        "CustomFunctions.StreamingHandler - result type number not created properly"
      );
      ...
      assert.strictEqual(
        j.functions[13].options.cancelable,
        true,
        "CustomFunctions.CancelableHandler type not created properly"
      );
      ...
      assert.strictEqual(
        j.functions[16].options.requiresAddress,
        true,
        "CustomFunctions.Invocation requiresAddress tag not created properly"
      );
      assert.strictEqual(
        j.functions[17].options.cancelable,
        true,
        "CustomFunctions.CancelableInvocation type not created properly"
      );
      assert.strictEqual(
        j.functions[17].options.requiresAddress,
        true,
        "CustomFunctions.CancelableInvocation requiresAdress type not created properly"
      );
      assert.strictEqual(
        j.functions[18].options.stream,
        true,
        "CustomFunctions.StreamingInvocation - options stream not created properly"
      );
      assert.strictEqual(
        j.functions[18].options.requiresAddress,
        undefined,
        "CustomFunctions.StreamingInvocation requiresAddress - should not be present"
      );
...
describe("test javascript file as input", function () {
  describe("js test", function () {
    it("basic test", async function () {
      ...
      assert.strictEqual(
        j.functions[5].options.stream,
        true,
        "CustomFunctions.StreamingHandler type any not created properly"
      );
      assert.strictEqual(j.functions[5].result.type, "string", "streaming result type any not created properly");
      assert.strictEqual(
        j.functions[6].options.cancelable,
        true,
        "CustomFunctions.CancelableHandler type any not created properly"
      );
      ...
      assert.strictEqual(
        j.functions[9].options.requiresAddress,
        true,
        "CustomFunctions.Invocation set requiresAddress not created properly"
      );
      assert.strictEqual(
        j.functions[10].options.stream,
        true,
        "CustomFunctions.StreamingInvocation type any not created properly"
      );
      assert.strictEqual(
        j.functions[11].options.cancelable,
        true,
        "CustomFunctions.CancelableInvocation type any not created properly"
      );
    });
  });
});
...
describe("test parseTreeResult", function () {
  describe("parseTreeResult", function () {
    it("parseTree for errorfunctions", async function () {
      ...
      assert.strictEqual(
        parseTreeResult.extras[5].errors[0].includes("привет".toLocaleUpperCase()),
        true,
        "Error message contains function name"
      );
      assert.strictEqual(
        parseTreeResult.extras[6].errors[0].includes("Duplicate function name"),
        true,
        "Error message contains duplicate function name"
      );
      assert.strictEqual(
        parseTreeResult.extras[7].errors[0].includes("@customfunction tag specifies a duplicate name"),
        true,
        "Error message contains duplicate function name from custom function"
      );
      assert.strictEqual(
        parseTreeResult.extras[9].errors[0].includes("@customfunction tag specifies a duplicate name"),
        true,
        "Duplicate name from custom function tags"
      );
      assert.strictEqual(
        parseTreeResult.extras[10].errors[0].includes("@customfunction tag specifies a duplicate id"),
        true,
        "Duplicate id from custom function tags"
      );
      assert.strictEqual(
        parseTreeResult.extras[11].errors[0].includes("@customfunction tag specifies a duplicate id"),
        true,
        "Duplicate id with function name from custom function tags"
      );
    });
  });
});
