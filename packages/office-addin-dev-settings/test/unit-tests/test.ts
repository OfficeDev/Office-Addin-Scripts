...
async function testSetSourceBundleUrlComponents(
  components: devSettings.SourceBundleUrlComponents,
  expected: devSettings.SourceBundleUrlComponents
) {
  ...
  if (isWindows || isMac) {
    ...
    describe("basic functionality", function () {
      ...
      it("Can register an add-in", async function () {
        ...
        const registeredManifestPath = isMac
          ? fspath.join(
              os.homedir(),
              "/Library/Containers/com.microsoft.Excel/Data/Documents/wef/6dd581d2-98d1-4eaf-9506-e0a24be515f5.manifest.xml"
            )
          : manifestPath;
    ...
    describe("multiple add-ins", function () {
      ...
      const secondRegisteredManifestPath = isMac
        ? fspath.join(
            os.homedir(),
            "/Library/Containers/com.microsoft.Excel/Data/Documents/wef/813cfc85-2a0f-49f6-8024-8d942cb73456.manifest2.xml"
          )
        : secondManifestPath;
    ...
    describe("json manifest functionality", function () {
      it("Error when file does not exist", () => {
        ...
        return registerWithTeams(zipPath).then(
          () => {
    ...
    if (isWindows) {
      describe("register Outlook add-in", function () {
        this.beforeEach(async function () {
          const key = await devSettingsWindows.getDeveloperSettingsRegistryKey(
            devSettingsWindows.OutlookSideloadManifestPath
          );
          if (key) {
            await deleteKey(key);
          }
        ...
        it("verify OutlookSideloadManifestPath key is not set and Develoer registry key is set", async function () {
          ...
          const key = devSettingsWindows.getDeveloperSettingsRegistryKey(
            devSettingsWindows.OutlookSideloadManifestPath
          );
  ...
  if (isWindows) {
    ...
    describe("enableRuntimeLogging", function () {
      ...
      it("default path but TEMP folder doesn't exist", async function () {
        ...
        let error;
        try {
          await devSettings.enableRuntimeLogging();
        } catch (err: any) {
          error = err;
        }
        ...
        assert.strictEqual(
          error.message,
          `You need to specify the path where the file can be written. Unable to write to: "${directoryDoesNotExist}\\${defaultFileName}".`
        );
      ...
      describe("filesystem test cases", async function () {
        ...
        it("directory does not exist", async function () {
          ...
          assert.strictEqual(
            error.message,
            `You need to specify the path where the file can be written. Unable to write to: "${filePath}".`
          );
        });
        ...
        it("file already exists but is not writable", async function () {
          ...
          assert.strictEqual(
            error.message,
            `You need to specify the path to a writable file. Unable to write to: "${filePath}".`
          );
        });
      });
    });
...
describe("Sideload to Desktop", function () {
  ...
  it("Sideload unsupported host (expect error)'", async function () {
    ...
    try {
      await devSettingsSideload.sideloadAddIn(
        manifestPath,
        OfficeApp.Project,
        true /* canPrompt */,
        AppType.Desktop,
...
describe("Sideload to web", function () {
  const docurl: string =
    "https://microsoft-my.sharepoint-df.com/personal/user_microsoft_com/_layouts/15/Doc.aspx?&file=Document.docx";
  const expectedQueryParams =
    "&wdaddindevserverport=8080&wdaddinmanifestfile=manifest.xml&wdaddinmanifestguid=6dd581d2-98d1-4eaf-9506-e0a24be515f5";
  ...
  it("Get sideload url with query params", async function () {
    ...
    const generatedUrl: string | undefined = await devSettingsSideload.generateSideloadUrl(
      manifestFileName,
      manifest,
      docurl
    );
